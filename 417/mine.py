...
realAnswers = [[0,4],[1,3],[1,4],[2,2],[3,0],[3,1],[4,0]]
...
class Solution:
  def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
    ...
    results = [[[None for _ in range(2)] for _ in range(cols)] for _ in range(rows)]
    ...
    def setPath(path, i):
      for p in path:
        ...
        if [y, x] not in answers and results[y][x] == [True, True]:
          answers.append([y,x])
    
    def isValidNeighbour(y1, x1, y2, x2):
      if y2 >= rows or y2 < 0 or x2 >= cols or x2 < 0:
        return False
      if f"{y2}-{x2}" in visited or None not in results[y2][x2]:
        if heights[y1][x1] >= heights[y2][x2] and None not in results[y2][x2]:
          if results[y2][x2][0] == True:
            results[y1][x1][0] = True
          if results[y2][x2][1] == True:
            results[y1][x1][1] = True

          if [y1, x1] not in answers and results[y1][x1] == [True, True]:
            answers.append([y1,x1])
        return False
      return heights[y1][x1] >= heights[y2][x2]
      
    def dfs(y, x, path):
      if f"{y}-{x}" in visited or None not in results[y][x]:
        if results[y][x][0] == True:
          setPath(path,0)
        if results[y][x][1] == True:
          setPath(path,1)
        return
      ...
      if (isValidNeighbour(y, x, y-1, x)):
        neighbours.append([y-1, x])
      if (isValidNeighbour(y, x, y+1, x)):
        neighbours.append([y+1, x])
      if (isValidNeighbour(y, x, y, x-1)):
      ...
      if results[y][x] == [True, True] and [y,x] not in answers:
        answers.append([y,x])
      
      for new_y, new_x in neighbours:
        dfs(new_y, new_x, path + [[new_y, new_x]])
      
      del visited[f"{y}-{x}"]
      
      results[y][x][0] = results[y][x][0] or False
      results[y][x][1] = results[y][x][1] or False
